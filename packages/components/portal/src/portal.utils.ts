import { isBrowser, warn, __DEV__ } from "@luniand-ui/utils";

let __portal_id__: number = 0;

export const createPortalTarget = (name: string = "default") => {
  const portalId = `luniand__${name}__portal__${__portal_id__}`;
  let target = document.getElementById(portalId);
  if (!target) {
    target = document.createElement("div");
    target.id = `luniand__${name}__portal__${__portal_id__++}`;
    /**
     * @todo
     *
...
export const ensureTarget = (selector: string) => {
  if (!isBrowser) {
    console.warn(
      `[luniand-ui:portal]: The CPortal component can only be used in the document`
    );
  }

  if (!document.querySelector(selector)) {
    console.warn(
      `[luniand-ui:portal]: the portal target "${selector}" cound not be found in the document. Portal children may not be rendered`
    );
  }
};
