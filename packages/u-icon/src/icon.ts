...
import { SNAO, camelCase } from "@uniland-ui/utils"
...
export interface IconProps
  extends Omit<SVGAttributes, keyof UnilandProps>,
    UnilandProps {
  /**
   * Icon Size
   */
...
export const UIcon: ComponentWithProps<DeepPartial<IconProps>> =
  defineComponent({
    name: "UIcon",
    props: _iconProps,
    setup(_props, { slots, attrs }) {
      const props = computed<IconProps>(() => mergeWith({}, iconProps, _props))
      const icons = inject<Record<string, any>>("$unilandIcons")
      const icon = computed(
